...
run() {
  rm moved.tf foo/moved.tf
  tfmv -R --regexp '^test-/example-'
}

clean() {
  git checkout -- main.tf moved.tf foo/main.tf foo/moved.tf
}

run_test() {
  for file in main.tf moved.tf foo/main.tf foo/moved.tf; do
